...
if __name__ == "__main__":
    ...
    ocr_inferencer = MMOCRInferencer(text_det_cfg_path, text_det_ckpt_path, text_rec_cfg_path, text_rec_ckpt_path, device=device)

    sam_model = sam_model_registry[sam_model_type](checkpoint=sam_ckpt_path)
    sam_model.to(device)
    sam_predictor = SamPredictor(sam_model)

    input_path_list = ocr_inferencer._inputs_ot_list(inputs_dir)
    ...
    for _, input_path in enumerate(input_path_list):
        ...
        detected_bboxes = torch.tensor(np.array([poly2bbox(detected_polygons) for polygon in detected_polygons]), device=device)
        transformed_bbox = sam_predictor.transform.apply_boxes_torch(detected_bboxes, input_img.shape[:2])

        sam_predictor.set_image(input_img, image_format="BGR")

        masks, _, _ = sam_predictor.predict_torch(point_coords=None, point_labels=None, boxes=transformed_bbox, multimask_output=False)
